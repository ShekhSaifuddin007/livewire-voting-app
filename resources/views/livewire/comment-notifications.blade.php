...
>
    ...
    >
        ...
        @if ($notificationCount)
            <div class="absolute rounded-full bg-red-500 text-white text-xxs w-6 h-6 flex justify-center items-center border-2 -top-1 -right-1">
                {{ $notificationCount }}
            </div>
        @endif
    </button>
    <ul
        class="absolute w-76 md:w-96 text-left text-gray-700 text-sm bg-white shadow-lg rounded-xl max-h-128 overflow-y-auto z-10 -right-28 md:right-0"
        x-cloak
    ...
    >
        ...
        @if ($notifications->isNotEmpty() && ! $isLoading)
            @foreach ($notifications as $notification)
                <li>
                    <a
                        href="{{ route('ideas.show', $notification->data['idea_slug']) }}"
                        @click.prevent="
                            isOpen = false
                        "
            ...
            <li class="border-t border-gray-300 text-center">
                <button
                    ...
                    class="w-full block focus:outline-none font-semibold hover:bg-gray-100 transition duration-150 ease-in px-5 py-4"
                >
                    Mark all as read
                </button>
            </li>
